...
class MixedMerton():
    ...
    def new_log_wealth_instance(self, x, control, dB, dN, compensator, i):
        ...
        tmp = control * dN[i, :, :]
        tmp[tmp < -1] = -0.99
        dx = ((1-control) * self.r + control * drift_comp - 0.5 * control**2 * torch.inner(volatility_t, volatility_t)) * self.dt + \
    ...
    def initial_value_MC(self, sample_size, R):
        ...
        jumps = DataGenerator.gen_jumps_mixture(R, sample_size, self.mus, self.sigmas, self.rates)
        jump_times = DataGenerator.gen_jump_times(R, sample_size,
                                                  [sum(self.rates)], self.T)
        dN = DataGenerator.gen_compound_poisson(R, sample_size, self.dim_N,
                                                torch.log(jumps+1), jump_times)
        ...
        s_T = self.s0*torch.exp((drift_comp - 0.5*torch.inner(volatility_t,volatility_t)) * self.T + \
              ...
              torch.sum(torch.sum(dN, dim=-1, keepdim=True), dim=0))
        ...
        ln_m1 = torch.exp(mus_t + 0.5*sigmas_t**2)-1 #first moment of log-normal distribution
...
fin_model = MixedMerton("MixedMerton",financial_parameters["T"],financial_parameters["drift"],
...
#Initialize the solver and train the model4
